...
```bash
# 1. Normalize the model output; p1-offset defaults to 4 (the PRSS55 convention)
...
- `has_multiple_isoforms`: whether the protein has multiple known isoforms
  (from UniProt `ALTERNATIVE PRODUCTS` comments);
- `subcellular_location`: UniProt-annotated subcellular compartment(s) for the
  protein (e.g. `Cell membrane`, `Secreted`, `Cytoplasm`);
