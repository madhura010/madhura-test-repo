...
A periodic checkpoint for the whole system. Not a pre-commit gate. A deliberate
audit that asks: does the system reflect our current understanding, and is it as
simple as it could be? It *delegates* the code-quality half to
`aganitha-code-review` (Step 4) and owns what that skill cannot see — vision,
drift, history, and time.
...
## Step 4 — Code review, whole-system

Do not re-derive code-quality checks here. `aganitha-code-review` owns them —
fit, structure, boundaries, breakage, and quality — and it takes a path as a
target.

Run it against the repository root as a standing review. Then:

Every finding it returns carries a disposition. Route by that, not by pass:

- **Apply** — local and reversible. This audit is allowed to fix them; do so.
- **Decide** — carry into this report under *Requires discussion*. Structural
  changes are decisions, not fixes.
- **Escalate** — needs access or authority you do not have here: a credential to
  rotate, history to purge, infrastructure to change. Put these at the top of the
  report with the action and the person who has to take it. Never attempt them.

The division of labour, and the test for anything added later: **if a check can
be answered from the code alone, it belongs to code-review. If it needs git
history, the docs, or the passage of time, it belongs here.**

(This delegation is sound because both skills ship in `dev-pack`. If they are
ever split across packs, revisit it — see the pack-dependency rule in
`AGENTS.md`.)
...
**Scope — keep this distinct.** `aganitha-code-review`'s Fit check asks whether a
*change* was larger than its ticket. This asks whether a *subsystem* is larger
than its actual usage — which the code cannot answer, because it depends on who
really calls it and how often. Same lens, different scale, different evidence.
...
Summary:
  ...
  [✓] Branches — none stale
  [✓] Code review — 3 applied, 1 for discussion, 0 escalated
